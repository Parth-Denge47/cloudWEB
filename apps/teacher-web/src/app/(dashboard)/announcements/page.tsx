...
export default function AnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      setIsLoading(true);
      const result = await announcementService.getAll({ tuitionId: teacher.tuitionId });
  ...
  return (
    <div>
      <PageHeader
        title="Announcements"
        description="Create, edit, and delete announcements for your students."
        actions={
          <Button
            onClick={() => {
          ...
          {filteredAnnouncements.map((announcement) => (
            <div key={announcement.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm shadow-slate-900/[0.02]">
              <div className="flex items-start justify-between gap-4">
                ...
                {announcement.createdByRole === "teacher" && announcement.createdByName === teacher.name && (
                  <RowActionsMenu
                    actions={[
                      {
                        label: "Edit",
