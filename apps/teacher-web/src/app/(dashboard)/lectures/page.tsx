...
export default function LecturesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      setIsLoading(true);
      const result = await lectureService.getAll({ tuitionId: teacher.tuitionId });
      setLectures(result);
      setIsLoading(false);
    })();
  }, [teacher]);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Unlisted YouTube lectures for your tuition — students only ever see the embedded player."
        actions={
          <Button
            onClick={() => {
      ...
      {isLoading ? (
        ...
        <EmptyState icon={VideoIcon} title="No lectures found" description="Upload your first lecture to get started." />
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filteredLectures.map((lecture) => (
            <div key={lecture.id} className="flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm shadow-slate-900/[0.02]">
              <button onClick={() => setPreviewLecture(lecture)} className="group relative flex aspect-video items-center justify-center bg-slate-900">
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
                  ...
                  {lecture.uploadedByRole === "teacher" && lecture.uploadedByName === teacher.name && (
                    <RowActionsMenu
                      actions={[
                        {
                          label: "Edit",
      ...
      <LectureFormModal key={formKey} open={formOpen} lecture={editingLecture} teacher={teacher} onClose={() => setFormOpen(false)} onSubmit={handleCreateOrUpdate} />
