...
// Quiz authoring UI for Admin — same builder Teachers use, but Admin
// additionally picks which Tuition + Teacher the quiz is authored under,
// matching "Admin has every power Teacher has, across every tuition."
export function QuizBuilder({ quiz, tuitions, teachers, onSave, onCancel }: QuizBuilderProps) {
  const [title, setTitle] = useState(quiz?.title ?? "");
  const [tuitionId, setTuitionId] = useState(quiz?.tuitionId ?? tuitions[0]?.id ?? "");
  const [teacherId, setTeacherId] = useState(quiz?.teacherId ?? "");
  ...
  const teachersForTuition = teachers.filter((t) => t.tuitionId === tuitionId);
  const effectiveTeacherId = teacherId || teachersForTuition[0]?.id || "";
  ...
  function validate(): string | null {
    ...
    if (!effectiveTeacherId) return "Select which teacher this quiz is authored under.";
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <TextField label="Quiz Title" required className="sm:col-span-2" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Rotational Motion Test" />
          <SelectField label="Tuition" required value={tuitionId} disabled={Boolean(quiz)} onChange={(e) => setTuitionId(e.target.value)}>
            {tuitions.map((t) => (
          ...
          <SelectField label="Authored By (Teacher)" required value={effectiveTeacherId} disabled={Boolean(quiz)} onChange={(e) => setTeacherId(e.target.value)}>
